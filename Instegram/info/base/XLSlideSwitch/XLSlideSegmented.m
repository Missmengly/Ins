...
//button标题选中大小
static const CGFloat ItemFontSize = 15.0f;
//最大放大倍数
...
//    _bottomLine.backgroundColor = [UIColor cyanColor];
...
- (void)layoutSubviews {
    ...
    _bottomLine.frame = CGRectMake(0, self.bounds.size.height, self.bounds.size.width, 1);
}
...
- (CGRect)shadowRectOfIndex:(NSInteger)index {
    return CGRectMake([_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]].frame.origin.x, self.bounds.size.height - 3, [self itemWidthOfIndexPath:[NSIndexPath indexPathForRow:index inSection:0]], 3);
}
